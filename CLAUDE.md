...
- **xxHash64 checksums by default**: checksum duplicate verification is enabled by default for safer duplicate detection; use `--no-checksum-duplicates` or `checksum_duplicates: false` for faster size/timestamp-only matching
