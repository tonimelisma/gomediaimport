...
5. `copyFiles()` — concurrent worker pool (default 4 workers), stream copy with `Sync()` + explicit `Close()` error check, post-copy xxHash64 verification by default, cleanup partial files on failure, progress tracking with ANSI sticky line
