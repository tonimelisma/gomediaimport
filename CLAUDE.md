...
- **main.go** — Entry point, `run(osArgs []string)` function (extracted from `main()` for testability, takes os args as parameter — no global state), CLI parsing via `arg.NewParser`+`Parse` (`go-arg`), strict YAML config loading (`KnownFields` rejects unknown keys), config validation, and `wasFlagProvided(osArgs, flag)` pure function for proper boolean override semantics. Three-tier config precedence: CLI flags > YAML file (`os.UserConfigDir()/gomediaimport/config.yaml`) > hardcoded defaults. `run()` dispatches one-off/source-directory imports, configured removable-volume imports, and `volumes` subcommands. Help output appends the resolved platform-specific config file path. `checksum_duplicates` defaults true; `--no-checksum-duplicates` opts out. `--quiet` flag suppresses all non-error stdout output (`cfg.Quiet` forces `cfg.Verbose=false`).
- **import.go** — Core orchestrator. Defines `FileStatus` type with typed constants and `FileInfo` struct (central data type tracking source/dest paths, checksums, creation time, optional `VideoMetadata`, media category, status). `importMedia()` coordinates enumeration → `planDestinations()` → `copyFiles()` → `deleteOriginalFiles()` → `cleanupSourceArtifacts()` → `printSummary()` → `ejectAfterImport()`. Planning, copy, original deletion, and artifact cleanup errors all stop later destructive phases and ejection. `planDestinations()` handles two-pass destination planning (non-sidecars then sidecars); date-time rename imports first sort by capture time and natural source filename order so same-second suffixes follow camera sequence order. `progressTracker` type encapsulates ANSI progress display with atomic size tracking.
- **file_operations.go** — File enumeration via `filepath.WalkDir` separates importable files from typed cleanup targets. Top-level system trash, exact Sony `[PRIVATE/]M4ROOT/THMBNL` directories, Sony `CLIP/*.XML`, and AppleDouble `._*` files never enter media classification. The file also handles duplicate detection (size+timestamp index or xxHash64 checksum), unique filenames, verified stream copies, and cross-platform disk eject.
- **removable_volumes.go / removable_volumes_*.go** — Label-based removable volume workflow. `volumes list` discovers currently mounted removable volumes, `volumes add` persists a mounted label in `removable_volumes`, and plain `gomediaimport` imports all mounted volumes whose labels are configured. Discovery is no-cgo: macOS uses `Getfsstat` plus `getattrlist`, accepting explicit `MNT_REMOVABLE` volumes and local `/Volumes/...` device mounts; Linux uses `/proc/self/mountinfo`, `/sys/dev/block`, and `/dev/disk/by-label`.
...
1. `run()` → parse config (defaults → YAML → CLI overrides via `wasFlagProvided`) and dispatch to volume subcommands, configured removable-volume import, or one-off/source-directory import
2. `enumerateFiles()` — WalkDir source dir, skip symlinks, prune system trash and exact Sony support paths, record cleanup targets, filter remaining files by media extensions, and extract timestamps
...
5. `copyFiles()` — concurrent worker pool (default 4 workers), stream copy with written-byte check and explicit `Close()` error handling, cleanup partial files on failure, progress tracking with ANSI sticky line
6. Optional: delete originals, then clean recorded trash/Sony/AppleDouble artifacts; any failure stops ejection
...
- **Label-only removable volume selectors**: `removable_volumes` stores labels only. The source path is computed from currently mounted removable volumes at runtime; duplicate mounted labels are all imported. `source_directory` remains legacy-compatible but configured removable volumes are preferred when present and `--source` is omitted.
...
- **xxHash64 checksums by default for duplicate candidates only**: checksum verification is used during planning when an existing destination file or previous source file already looks like a duplicate by size/timestamp. Copies are not checksummed after transfer.
...
- **Source artifacts never import**: recognized system trash, Sony XAVC thumbnail/XML support data, and AppleDouble files are excluded before extension classification. `delete_originals` removes them only after copying and ordinary original deletion succeed.
- **Strict configuration**: unknown or removed YAML keys fail parsing rather than being ignored.
