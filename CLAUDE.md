...
- **file_operations.go** — File enumeration via `filepath.WalkDir` (with symlink skipping), duplicate detection (size+timestamp index or xxHash64 checksum), `exists()` returns `(bool, error)`, unique filename generation (appends `_001` through `_999999` on conflicts), `copyFile()` stream copy with written-byte check and explicit `Close()` error handling, cross-platform disk eject (`ejectDrive` dispatches to `ejectDriveDarwin` on macOS or `ejectDriveLinux` on Linux).
